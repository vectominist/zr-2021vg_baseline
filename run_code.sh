...
lab_dir=~/scratch/exp/pnmi_exp/code_lab/speech_dino_$layer
exp_dir=~/scratch/exp/slm_exp/speech_dino_${layer}_bz20k
...
if [ $stage -le 3 ] && [ $end_stage -ge 3 ]; then
    ...
    SPAN_SIZE=10 # equivalent to 100 ms
    MAX_TOKENS=20000  # 16384
    fairseq-train --fp16 \
        $exp_dir/data \
        ...
        --distributed-world-size 2 --distributed-port 18806 \
...
#     if [ $skip_5 -le 0 ]; then
#         zerospeech2021-evaluate $corpus_dir $exp_dir/submission_${M}_${Dt} -o $exp_dir/result_${M}_${Dt} --no-phonetic
#     else
#         zerospeech2021-evaluate $corpus_dir $exp_dir/submission_${M}_${Dt} -o $exp_dir/result_${M}_${Dt} --no-phonetic --no-semantic
#     fi
